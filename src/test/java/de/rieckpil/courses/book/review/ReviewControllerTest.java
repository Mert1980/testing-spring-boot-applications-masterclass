...
class ReviewControllerTest {
  ...
  void shouldCreateNewBookReviewForAuthenticatedUserWithValidPayload() throws Exception {
    ...
    when(reviewService.createBookReview(eq("42"), any(BookReviewRequest.class), eq("duke"), endsWith("spring.io")))
      .thenReturn(84L);
    ...
    this.mockMvc.perform(
        post("/api/books/{isbn}/reviews", 42)
          ...
          .content(requestBody)
          .with(jwt()
            .jwt(builder -> builder.claim("email", "duke@spring.io")
  ...
  void shouldAllowDeletingReviewsWhenUserIsAuthenticatedAndHasModeratorRole() throws Exception {
    this.mockMvc.perform(delete("/api/books/{isbn}/reviews/{reviewId}", 42, 3))
      // .with(jwt().authorities(new SimpleGrantedAuthority("ROLE_moderator")))) // Alternative voor @WithMockUser
      .andExpect(status().isOk());
